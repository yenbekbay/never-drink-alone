...
CGFloat const kActionSheetAutoDismissOffset = 80;
// Length of the range at which the blurred background is being hidden when the user scrolls the tableView to the top.
CGFloat const kActionSheetBlurFadeRangeSize = 200;
// Cancel button's shadow height as the ratio to the cancel button's height
CGFloat const kActionSheetCancelButtonShadowHeightRatio = 0.3f;
CGFloat const kActionSheetDisabledAlpha = 0.3f;
// Offset at which there's a check if the user is flicking the tableView down.
CGFloat const kActionSheetFlickDownHandlingOffset = 20;
CGFloat const kActionSheetFlickDownMinVelocity = 2000;
...
CGFloat const kActionSheetTopSpaceMarginFraction = 0.3f;
NSTimeInterval const kActionSheetAnimationDuration = 0.4f;
UIEdgeInsets const kActionSheetHeaderViewLabelPadding = {
  10, 15, 10, 15
};

@interface NDAActionSheetItem : NSObject
...
@implementation NDAActionSheetItem
@end


@interface NDAActionSheetViewController : UIViewController

@property (nonatomic) NDAActionSheet *actionSheet;

@end

@implementation NDAActionSheetViewController
...
@interface NDAActionSheet () <UITableViewDataSource, UITableViewDelegate>
...
- (void)addButtonWithTitle:(NSString *)title image:(UIImage *)image handler:(NDADismissHandler)handler {
  NDAActionSheetItem *item = [NDAActionSheetItem new];
...
- (void)show {
  ...
  void (^delayedAnimations)(void) = ^(void) {
    ...
    for (NDAActionSheetItem *item in self.items) {
      if (item.view) {
    ...
    CGFloat topInset;
    BOOL buttonsFitInWithoutScrolling = tableContentHeight <
      self.tableView.height * (1 - kActionSheetTopSpaceMarginFraction);
    if (buttonsFitInWithoutScrolling) {
      // show all buttons if there isn't many
      topInset = self.tableView.height - tableContentHeight;
    } else {
      // leave an empty space on the top to make the control look similar to UINDAActionSheet
...
- (void)dismissAnimated:(BOOL)animated duration:(NSTimeInterval)duration handler:(NDADismissHandler)handler {
  if (![self isVisible]) {
    return;
  }
  ...
  void (^tearDownView)(void) = ^(void) {
    [@[self.tableView, self.cancelButton, self.backgroundView, self.window] makeObjectsPerformSelector : @selector(removeFromSuperview)];
    self.window = nil;
...
- (void)setUpNewWindow {
  NDAActionSheetViewController *actionSheetViewController = [NDAActionSheetViewController new];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  NDAActionSheetItem *item = self.items[(NSUInteger)indexPath.row];
  if (!item.view) {
    ...
    if ((NSInteger)self.activeIndex == indexPath.row || (NSInteger)self.staticIndex == indexPath.row) {
      ...
      if ((NSInteger)self.staticIndex == indexPath.row) {
        ...
        cell.textLabel.alpha = kActionSheetDisabledAlpha;
        checkIcon.alpha = kActionSheetDisabledAlpha;
      }
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (!item.view && indexPath.row != (NSInteger)self.staticIndex) {
