...
- (DZNPhotoEditorContainerView *)bottomView {
    if (!_bottomView) {
        ...
        _leftButton = [self buttonWithTitle:NSLocalizedString(@"Отмена", nil)];
        [_leftButton addTarget:self action:@selector(cancelEdition:) forControlEvents:UIControlEventTouchUpInside];
        
        _rightButton = [self buttonWithTitle:NSLocalizedString(@"Выбрать", nil)];
        ...
        if (_cropMode == DZNPhotoEditorViewControllerCropModeCircular) {
            ...
            topLabel.text = NSLocalizedString(@"Двигайте и масштабируйте", nil);
...
- (CGSize)cropSize {
    ...
    if (self.cropMode == DZNPhotoEditorViewControllerCropModeCustom) {
        CGFloat cropHeight = roundf((float)((_cropSize.height * viewSize.width) / _cropSize.width));
...
- (UIImage *)editedImage {
    ...
    if (self.cropMode == DZNPhotoEditorViewControllerCropModeCircular) {
        ...
        CGFloat increment = (CGFloat)(1.f/(((self.innerInset*2)*100.0)/viewRect.size.width));
        CGFloat scale = (CGFloat)(1.f + round(increment * 10) / 10.0);
...
- (UIImage *)trimmedImage:(UIImage *)image {
    ...
    CGPoint left = CGPointZero;
    CGPoint right = CGPointZero;
    CGPoint bottom = CGPointZero;
    
    BOOL breakOut = NO;
    for (int x = 0;breakOut==NO && x < (int)width; x++) {
        for (int y = 0; y < (int)height; y++) {
            NSInteger loc = x + (y * (int)width);
            loc *= 4;
    ...
    breakOut = NO;
    for (int y = 0;breakOut==NO && y < (int)height; y++) {
    ...
    breakOut = NO;
    for (NSInteger y = (int)height-1;breakOut==NO && y >= 0; y--) {
        
        for (NSInteger x = (int)width-1; x >= 0; x--) {
    ...
    breakOut = NO;
    for (NSInteger x = (int)width-1;breakOut==NO && x >= 0; x--) {
        
        for (NSInteger y = (int)height-1; y >= 0; y--) {
...
- (void)updateScrollViewContentInset {
    ...
    CGFloat vInset = fabsf((float)(maskHeight-imageSize.height)/2);
